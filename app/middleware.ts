...
export async function middleware(request: NextRequest) {
  ...
  if (token) {
    // If authenticated, clear any guest cookies to avoid conflicts
    const response = NextResponse.next();
    
    // Check if there are guest cookies to clear
    const cookies = request.cookies;
    if (cookies.has('guestMode')) {
      response.cookies.delete('guestMode');
      // Also clear guestToken if it exists
      if (cookies.has('guestToken')) {
        response.cookies.delete('guestToken');
      }
      
      console.log('Cleared guest cookies for authenticated user');
    }
